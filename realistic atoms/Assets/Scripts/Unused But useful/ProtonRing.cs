...
//    [Range(3, 360)]
//    public int segments = 3; //how many segments the Shell is made up of. the more segments, the smoother/rounder the edge. in general, no need to be so many as 360 but as a circle. has to be set to 3 otherwise range wont be set and will default to zero leading to issues
...
//    public float thickness = 0.5f; //how wide the Shell will be 
...
//        ring = new GameObject(name + " Ring"); //sets Shell variable to a new game object. name in this case for the minute is just nucleus. so therefore "nucleus Shell". this allows us to know which Shell is associated with what object in the hierarchy
//        ring.transform.parent = transform; //parents the Shell to the nucleus/object
...
//        ring.transform.localPosition = Vector3.zero; //resets the position so that it starts at the zero (or the centre of) where ever the planet is positioned. Shell is in the same position as the object
...
//        ringMF = ring.AddComponent<MeshFilter>(); //take Shell and add a component, returning the same component, allowing us to add the mesh filter and assign at the same time 
...
//        ringMR.material = ringMat; //makes sure the mesh renderer is set the Shell material that assigned in the inspector
...
//        Vector3[] vertices = new Vector3[(segments + 1) * 2 * 2]; //vector 3 array (holds x,y,z points) equal to a new array which has a size of the number of segemnts we have +1 (as the very starting point of the Shell will also have to double up as the end position as we dont want them to be the same uv co-ordinates). we multiply by two as we need a vertex at the inner radius and outer final thickness and multiple by two again as we need vertices at the top of the Shell and at the bottom of the Shell.
...
//            vertices[i * 2] = vertices[i * 2 + halfway] = new Vector3(x, 0f, z) * (innerRadius + thickness); //this gives us the outside vertices of the Shell 
...
//            uv[i * 2 + 1] = uv[i * 2 + 1 + halfway] = new Vector2(progress, 1f); //for the inner edge of the Shell, really just reflecting the vertices. now this is all the way to the top (1f)
...
//        //this Shell mesh vertices array is likely the one we are going to need to use. 
